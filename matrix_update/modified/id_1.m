function [sk, rd, T, niter] = id_1(A, rank_or_tol, Tmax, rrqr_iter, fixed)
  ...
  if nfix > 0
    ...
    A = A(:,free);
    R2 = Q' * A; A = A - Q * R2;
    n = numel(A(1,:));  % update number of columns
...
% [~, R, p] = qr(A, 0);
  ...
  diagR = diag(R);              % extract diagonal elements
  ...
  if ~isinf(Tmax) && rrqr_iter > 0 && k > 0 && k < n
    ...
    while niter < rrqr_iter
      ...
      [t2, idx] = max(abs(tmp(:)));
      if t2 <= f2, conv = true; break; end  % converged
      niter = niter + 1;
      ...
      % update T and R
      if numel(R(:,1)) == k  % no trailing R22 -- typical for fixed-precision case
