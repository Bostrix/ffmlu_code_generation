                ...
                real_T x_data[], const int32_T x_size[2], int32_T col)
{
  ...
  scalarLB = (static_cast<uint8_T>(n) >> 1) << 1;
  vectorUB = scalarLB - 2;
              ...
              int32_T u_size, const real_T v_data[], int32_T v_size)
{
  ...
  if (u_size == 0) {
    b_m = q_size[1];
    if (b_m - 1 >= 0) {
      std::memset(&w_data[0], 0, static_cast<uint32_T>(b_m) * sizeof(real_T));
    }
  } else {
    TRANSB1 = 'N';
    TRANSA1 = 'T';
  ...
  for (int32_T j{0}; j < n_tmp; j++) {
    i = j + 2;
    for (int32_T b_i{i}; b_i <= b_m; b_i++) {
      r_data[(b_i + r_size[0] * j) - 1] = 0.0;
    }
  }
  mm1_tmp = q_size[0] - 1;
  for (int32_T j{mm1_tmp}; j >= 1; j--) {
    c_data[j - 1] =
  ...
  for (int32_T j{0}; j < n_tmp; j++) {
    ...
    for (int32_T b_i{b_m + 1}; b_i >= 1; b_i--) {
  ...
  for (int32_T j{0}; j < n_tmp; j++) {
    ...
    st.site = &gg_emlrtRSI;
    if (j + 1 > m) {
      b_m = static_cast<uint8_T>(m - 1);
    } else {
      b_m = static_cast<uint8_T>(j);
    }
    for (int32_T b_i{0}; b_i < b_m; b_i++) {
      rij_tmp = b_i + i;
  ...
  st.site = &fg_emlrtRSI;
  i = static_cast<uint8_T>(muIntScalarMin_sint32(mm1_tmp, n_tmp));
