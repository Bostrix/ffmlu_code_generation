         ...
         real_T y_data[], int32_T y_size[2])
{
  ...
  if (pipk - 1 >= 0) {
    std::memset(&y_data[0], 0, static_cast<uint32_T>(pipk) * sizeof(real_T));
  }
  b_st.site = &kf_emlrtRSI;
  x_size_idx_0 = x_size[0];
  if (pipk - 1 >= 0) {
    std::copy(&x_data[0], &x_data[pipk], &b_x_data[0]);
  }
  ...
  pipk = static_cast<uint8_T>(x_size[0] - 1) + 1;
  p_data[0] = 1;
  yk = 1;
  for (int32_T k{2}; k <= pipk; k++) {
    yk++;
    p_data[k - 1] = static_cast<int16_T>(yk);
  }
  for (int32_T k{0}; k < tmp_size; k++) {
    ...
    if (i > k + 1) {
      pipk = p_data[i - 1];
      p_data[i - 1] = p_data[k];
      p_data[k] = static_cast<int16_T>(pipk);
    }
  }
  ...
  if ((n1x == 0.0) || (n1xinv == 0.0) || (rc == 0.0)) {
    ...
    emlrt_marshallIn(c_st,
                     b_sprintf(c_st, y, emlrt_marshallOut(rc), f_emlrtMCI),
