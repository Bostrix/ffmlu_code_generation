...
static emlrtRSInfo emlrtRSI{
    ...
    "ffmlu_code_generation/Afun_lap_te/Afun_lap_te.m" // pathName
};
...
static emlrtRSInfo e_emlrtRSI{
    ...
    "/Users/abidrk/LocalStorage/Projects/ffmlu_optimize/fn-codegen/"
    "ffmlu_code_generation/Afun_lap_te/lap_neumann_kernel.m" // pathName
};
...
static emlrtRSInfo f_emlrtRSI{
    ...
    "/Users/abidrk/LocalStorage/Projects/ffmlu_optimize/fn-codegen/"
    "ffmlu_code_generation/Afun_lap_te/spget_quadcorr.m" // pathName
};
...
static emlrtBCInfo i_emlrtBCI{
    ...
    "Afun_lap_te", // fName
    "/Users/abidrk/LocalStorage/Projects/ffmlu_optimize/fn-codegen/"
    "ffmlu_code_generation/Afun_lap_te/Afun_lap_te.m", // pName
    0                                                  // checkKind
};
                 ...
                 const real_T contrast[8000], real_T wuse, real_T A[4095])
{
  ...
  for (mid = 0; mid < 5; mid++) {
    std::copy(&i[0], &i[819], &SD->f0.b_I[mid * 819]);
  ...
  for (mid = 0; mid < 5; mid++) {
    ...
    for (left = 0; left < 819; left++) {
      nx = left + 819 * mid;
      SD->f0.dx[nx] = b_x[3 * left] - y_tmp;
      SD->f0.dy[nx] = b_x[3 * left + 1] - d;
      dz[nx] = b_x[3 * left + 2] - d1;
    }
  }
  for (mid = 0; mid < 4095; mid++) {
    y_tmp = SD->f0.dx[mid];
    dr[mid] = y_tmp * y_tmp;
    y_tmp = SD->f0.dy[mid];
    SD->f0.c[mid] = y_tmp * y_tmp;
    y_tmp = dz[mid];
    M[mid] = y_tmp * y_tmp;
  }
  ...
  p = false;
  for (mid = 0; mid < 4095; mid++) {
    y_tmp = (dr[mid] + SD->f0.c[mid]) + M[mid];
    dr[mid] = y_tmp;
    if (p || (y_tmp < 0.0)) {
      p = true;
    }
  ...
  for (mid = 0; mid < 4095; mid++) {
    dr[mid] = muDoubleScalarSqrt(dr[mid]);
  }
  for (mid = 0; mid < 5; mid++) {
    for (left = 0; left < 819; left++) {
      nx = left + 819 * mid;
      SD->f0.c[nx] = nuuse[3 * left] * SD->f0.dx[nx];
      SD->f0.dx[nx] = nuuse[3 * left + 1] * SD->f0.dy[nx];
      M[nx] = nuuse[3 * left + 2] * dz[nx];
    }
  }
  for (mid = 0; mid < 4095; mid++) {
    y_tmp = dr[mid];
    SD->f0.c[mid] = -0.079577471545947673 *
                    ((SD->f0.c[mid] + SD->f0.dx[mid]) + M[mid]) /
  ...
  for (mid = 0; mid < 5; mid++) {
    for (left = 0; left < 819; left++) {
      nx = left + 819 * mid;
      A[nx] = SD->f0.c[nx] * area[static_cast<int32_T>(j[mid]) - 1];
    }
  }
  for (mid = 0; mid < 4095; mid++) {
    y_tmp = 2.0 * A[mid];
  ...
  for (mid = 0; mid < 5; mid++) {
    for (left = 0; left < 819; left++) {
      nx = left + 819 * mid;
      A[nx] = contrast[static_cast<int32_T>(i[left]) - 1] * M[nx];
    }
  }
  st.site = &b_emlrtRSI;
  ...
  r.set_size(&u_emlrtRTEI, &st, right);
  left = 0;
  for (int32_T b_i{0}; b_i <= nx; b_i++) {
    if (idx[b_i] > 0.0) {
      r[left] = b_i;
  ...
  for (mid = 0; mid < nx; mid++) {
    if (r[mid] > b_I.size(0) - 1) {
      emlrtDynamicBoundsCheckR2012b(r[mid], 0, b_I.size(0) - 1, &d_emlrtBCI,
                                    &st);
    }
  }
  for (mid = 0; mid < nx; mid++) {
    if (r[mid] > jj.size(0) - 1) {
      emlrtDynamicBoundsCheckR2012b(r[mid], 0, jj.size(0) - 1, &e_emlrtBCI,
                                    &st);
    }
  }
  for (mid = 0; mid < nx; mid++) {
    if (r[mid] > v.size(0) - 1) {
      emlrtDynamicBoundsCheckR2012b(r[mid], 0, v.size(0) - 1, &f_emlrtBCI, &st);
    }
  }
  for (mid = 0; mid < nx; mid++) {
    right = b_I[r[mid]];
  ...
  idx.set_size(&v_emlrtRTEI, &st, r.size(0));
  for (mid = 0; mid < nx; mid++) {
    idx[mid] = (static_cast<real_T>(jj[r[mid]]) - 1.0) * 819.0;
  }
  if (r.size(0) == idx.size(0)) {
    jj.set_size(&w_emlrtRTEI, &st, r.size(0));
    for (mid = 0; mid < nx; mid++) {
      y_tmp = SD->f0.P[b_I[r[mid]] - 1] + idx[mid];
    ...
    binary_expand_op(b_st, b_I, SD->f0.P, r, idx);
  }
  loop_ub = r.size(0);
  if (b_I.size(0) != r.size(0)) {
    emlrtSubAssignSizeCheck1dR2017a(b_I.size(0), r.size(0), &emlrtECI, &st);
  }
  for (mid = 0; mid < loop_ub; mid++) {
    M[b_I[mid] - 1] = v[r[mid]];
  }
  ...
  for (mid = 0; mid < 4095; mid++) {
    y_tmp = -2.0 * M[mid];
    M[mid] = y_tmp;
    if (muDoubleScalarAbs(y_tmp) != 0.0) {
      right++;
      tmp_data[left] = static_cast<int16_T>(mid);
      left++;
    }
  }
  ...
  p = false;
  for (mid = 0; mid < 819; mid++) {
    y_tmp = area[static_cast<int32_T>(i[mid]) - 1];
    b_y[mid] = y_tmp;
    if (p || (y_tmp < 0.0)) {
      p = true;
    }
  ...
  for (mid = 0; mid < 819; mid++) {
    b_y[mid] = muDoubleScalarSqrt(b_y[mid]);
  }
  ...
  p = false;
  for (mid = 0; mid < 5; mid++) {
    y_tmp = area[static_cast<int32_T>(j[mid]) - 1];
    b_area[mid] = y_tmp;
    if (p || (y_tmp < 0.0)) {
      p = true;
    }
  ...
  for (mid = 0; mid < 5; mid++) {
    b_area[mid] = muDoubleScalarSqrt(b_area[mid]);
  }
  for (mid = 0; mid < 5; mid++) {
    b_area[mid] = 1.0 / b_area[mid];
  }
  std::copy(&A[0], &A[4095], &M[0]);
  for (mid = 0; mid < 5; mid++) {
    for (left = 0; left < 819; left++) {
      nx = left + 819 * mid;
      A[nx] = M[nx] * b_area[mid];
    }
  }
