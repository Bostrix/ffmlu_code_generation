                   ...
                   array<real_T, 2U> &b)
{
  ...
  na_tmp = a.size(1);
  n = a.size(1) + 1;
  idx.set_size(&x_emlrtRTEI, &sp, 1, a.size(1));
  for (i = 0; i < na_tmp; i++) {
    idx[i] = 0;
  }
  if (a.size(1) != 0) {
    iwork.set_size(&y_emlrtRTEI, &sp, a.size(1));
    i = a.size(1) - 1;
    for (k = 1; k <= i; k += 2) {
      x = a[k];
  ...
  b.set_size(&ab_emlrtRTEI, &sp, 1, a.size(1));
  for (k = 0; k < na_tmp; k++) {
    b[k] = a[idx[k] - 1];
  }
  k = 0;
  ...
  b.set_size(&bb_emlrtRTEI, &sp, b.size(0), nb);
