function F = srskelf_asym_new_test(A, x, occ, rank_or_tol, pxyfun, opts)
    ...
    for lvl = t.nlvl:-1:1
      ...
      F.factors(n).nbr = nbr;   
            ...
            % Ensure that the index i is valid before updating lookup_list
            if i > 0 && i <= numel(t.nodes)
...
function A = spget(Ityp,Jtyp)
    ...
    for idx = 1:length(update_list) % Use an explicit index for the loop
        ...
        if strcmpi(Ityp,Jtyp)
            % For diagonal block
            idxI = round(find_locations_t(xj,I_)); % Convert to integers
            idxI = idxI(idxI >= 1 & idxI <= size(A, 1)); % Ensure valid indices
            tmp1 = double(idxI ~= 0); % Convert to double to ensure consistent types
            subI = idxI(tmp1);

            % Ensure f does not exceed the length of tmp1
            if f > length(tmp1)
                f = length(tmp1);
            end
            ...
            idxI = round(find_locations_t(xj,I_)); % Convert to integers
            idxI = idxI(idxI >= 1 & idxI <= size(A, 1)); % Ensure valid indices
            idxJ = round(find_locations_t(xj,J_)); % Convert to integers
            idxJ = idxJ(idxJ >= 1 & idxJ <= size(A, 2)); % Ensure valid indices
            ...
            % Ensure f does not exceed the length of tmp1 and tmp2
            if f > length(tmp1)
                f = length(tmp1);
            end
            if f > length(tmp2)
